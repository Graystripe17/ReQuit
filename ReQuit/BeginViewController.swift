...
class BeginViewController: UIViewController {
    ...
    @IBOutlet weak var beginLoadingIcon: UIActivityIndicatorView!
    ...
    @IBAction func didTapSignUp(_ sender: UIButton) {
        ...
        { (user, error) in
            if let error = error {
                ...
                self.beginLoadingIcon.stopAnimating()
                return
            }
        ...
        beginLoadingIcon.startAnimating()
        
    }
    
    func signUpFailed(error: Error) {
        print("The signUp failed" + error.localizedDescription)
    }
